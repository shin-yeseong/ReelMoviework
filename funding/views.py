...
def upload_funding_movie(request):
    ...
    if not session_id:
        return redirect('common:signin')  # 로그인 페이지로 리디렉션

    # MongoDB에서 세션 확인
    session = sessions_collection.find_one({"_id": session_id})
    if not session:
        return redirect('common:signin')  # 세션 만료 또는 존재하지 않을 경우 로그인 페이지로 리디렉션

    # 사용자 정보 가져오기
    user = users_collection.find_one({"_id": ObjectId(session['user_id'])})
    if not user:
        return redirect('common:signin')  # 사용자 정보가 없으면 로그인 페이지로 리디렉션
